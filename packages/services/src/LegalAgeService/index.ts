...
const getLegalAgeRange = (
	...
	const maximumAllowedBirthDateToRegister = dateSubtractingYears(
		new Date(),
		legalAge.minorThreshold
	);

	const maximumAllowedBirthDateToLegalConsent = dateSubtractingYears(
		new Date(),
		legalAge.minorThresholdWithGuardian
	);
	...
	const maximumAllowedBirthDateTimeStamp =
		maximumAllowedBirthDateToRegister.getTime();

	const maximumAllowedBirthDateToLegalConsentTimeStamp =
		maximumAllowedBirthDateToLegalConsent.getTime();

	if (dateInputTimeStamp > maximumAllowedBirthDateTimeStamp) {
		return LegalAgeRange.MINOR_UNABLE_TO_REGISTER;
	} else if (
		dateInputTimeStamp < maximumAllowedBirthDateTimeStamp &&
