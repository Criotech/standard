...
const convertMetersIntoKilometers = (meters: number): number => {
	const METER_KILOMETER_RATIO = 1000;
	return meters / METER_KILOMETER_RATIO;
};

const computeDistanceInKmBeetween = (
	point1: ICoordinates,
	point2: ICoordinates
): number => {
	...
	try {
		if (lat1 === lat2 && lon1 === lon2) {
			...
			const distanceInKilometers =
				convertMetersIntoKilometers(distanceInMeters);
			const FIXED_DECIMAL_PLACES = 2;

			return parseFloat(
				distanceInKilometers.toFixed(FIXED_DECIMAL_PLACES)
			);
		}
	} catch (ex) {
		return 0;
